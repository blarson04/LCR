...
**Diagnosis:** the migration linchpin is fine (M1 PEP-only substitution ≈ 0.98 Spearman); the loss comes from carrying forward the slow indicators — especially the *growth-rate* ones (`job_growth`, `income_growth`), which don't persist year to year. The likely fix is fresh employment data (CES), deferred because BLS metro employment needs a bot-blocked area-code crosswalk. **Future work:** source CES (or an accessible equivalent) and re-run M3; the nowcast's viability hinges on fresh jobs data, not on migration.
